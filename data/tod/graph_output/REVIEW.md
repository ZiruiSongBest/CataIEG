...
给化学专家合作者的使用说明。本轮更新重新设计了反应（Reaction）和催化剂（Catalyst）的归一化流程——从字符串匹配改为 LLM 语义归一化 + 二次合并。请协助审查归一化的化学合理性。
...
```
问题类型：错误合并 / 错误拆分
涉及家族：<family canonical_name>
涉及 Catalyst（uid 或 name_reported）：<列出>
为什么错：<一句话说明化学依据>
```

**格式 B（共现边不合理）：**
```
边：<source> --<edge_type>--> <target>
witness papers 数：<N>
为什么不合理：<一句话>
```
...
如果你觉得提示词本身有问题或需要加 / 改规则，告诉我，下次跑就能用上。
